...
with st.sidebar:
    ...
    device_display = {d: d for d in device_options}
    selected_device = st.selectbox(
        "设备",
        device_options,
        format_func=lambda x: f"{x} {'(自动检测)' if x == 'auto' else ''}"
    )
    ...
    if st.button("🚀 加载模型"):
        with st.spinner("加载中..."):
            if model_type == "huggingface":
                success, msg = load_model(model_type, model_name=model_name, device=selected_device)
            else:
                success, msg = load_model(model_type, checkpoint_path=checkpoint_path, device=selected_device)
            if success:
                st.success(msg)
            else:
                st.error(msg)
    ...
    if st.session_state.model_loaded:
        ...
        st.markdown(f"- 当前设备: {st.session_state.model_loader.current_device if st.session_state.model_loader else 'N/A'}")
